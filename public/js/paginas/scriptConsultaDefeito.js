...
if ("ontouchend" in document)
	document
			.write("<script src='#{request.contextPath}/js/jquery.mobile.custom.min.js'>"
					+ "<" + "/script>");
...
$(function() {
	...
	$("#input-span-slider").slider(
			{
				...
				slide : function(event, ui) {
					var val = parseInt(ui.value);
					$('#form-field-5').attr('class', 'span' + val).val(
							'.span' + val).next().attr('class',
							'span' + (12 - val)).val('.span' + (12 - val));
				}
			});
			...
			.slider(
					{
						...
						slide : function(event, ui) {
							...
							if (!ui.handle.firstChild) {
								$(ui.handle)
										.append(
												"<div class='tooltip right in' style='display:none;left:15px;top:-8px;'><div class='tooltip-arrow'></div><div class='tooltip-inner'></div></div>");
							}
	...
	// dynamically change allowed formats by changing before_change callback
	// function
	$('#id-file-format')
			.removeAttr('checked')
			.on(
							...
							before_change = function(files, dropped) {
								...
								for ( var i = 0; i < files.length; i++) {
									...
									if (typeof file === "string") {
										// IE8 and browsers that don't support
										// File Object
										...
										if ((type.length > 0 && !(/^image\/(jpe?g|png|gif|bmp)$/i)
											...
											continue;// not an image so don't
														// keep this file
									}
	...
	/**
	 * //or you can activate the chosen plugin after modal is shown //this way
	 * select element has a width now and chosen works as expected
